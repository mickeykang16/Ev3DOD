...
class DataProcessor(object):
    ...
    def mask_points_and_boxes_outside_range(self, data_dict=None, config=None):
        ...
        if data_dict.get('gt_boxes', None) is not None and config.REMOVE_OUTSIDE_BOXES and self.training:
            ...
            gt_len_list = []
            gt_len = 0
            for i in range(len(data_dict['gt_boxes'])):
                gt_data = data_dict['gt_boxes'][i]
                mask = box_utils.mask_boxes_outside_range_numpy(
                    gt_data, self.point_cloud_range, min_num_corners=config.get('min_num_corners', 1)
                )
                gt_data = gt_data[mask]
                gt_len += len(gt_data)
                gt_boxes_lidar_list.append(gt_data)
                gt_len_list.append(gt_len)
            data_dict['gt_boxes'] = gt_boxes_lidar_list
            data_dict['gt_len'] = gt_len_list                
        return data_dict

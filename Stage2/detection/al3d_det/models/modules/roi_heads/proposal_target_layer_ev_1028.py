...
class ProposalTargetLayerEV(nn.Module):
    ...
    def forward(self, batch_dict, index):
        ...
        prev_targets_dict = batch_dict['targets_dict']
        # for batch_idx in range(batch_size):
        cur_batch_gt_ids = prev_targets_dict['batch_gt_ids']
        id_index = torch.where(cur_batch_gt_ids.unsqueeze(-1) == gt_boxes[:, :, 9].unsqueeze(1))
        # id_index = torch.stack([id_index[0], id_index[2]], dim=1).reshape(batch_size, -1, 2)
        cur_gt_rois = torch.zeros((batch_size, cur_batch_gt_ids.shape[1], 7)).to(cur_batch_gt_ids.device)
        cur_gt_rois[id_index[0], id_index[1]] = gt_boxes[id_index[0], id_index[2]][:, :7]
        
        reg_valid_mask = torch.zeros_like(prev_targets_dict['reg_valid_mask'])
        reg_valid_mask[id_index[0], id_index[1]] = prev_targets_dict['reg_valid_mask'][id_index[0], id_index[1]]
        
        
        # prev_targets_dict['gt_of_rois'][batch_idx] = cur_gt_rois 
        
        targets_dict = {'rois': prev_targets_dict['rois'], 
                        'gt_iou_of_rois': prev_targets_dict['gt_iou_of_rois'],
                        'roi_scores': prev_targets_dict['roi_scores'], 
                        'roi_labels': prev_targets_dict['roi_labels'],
                        'reg_valid_mask': prev_targets_dict['reg_valid_mask'],
                        'rcnn_cls_labels': prev_targets_dict['rcnn_cls_labels'],
                        'gt_of_rois': cur_gt_rois
                        }
        
        return targets_dict

    def sample_rois_for_rcnn(self, batch_dict, time_idx):
        """
        Args:
            batch_dict:
                batch_size:
        ...
        gt_boxes = batch_dict['gt_boxes'][time_idx]
        ...
        for index in range(batch_size):
            ...
            # # print("######### batch_roi #########")
            # # print(batch_rois.shape)
            # # print(cur_roi.shape)
            # # print("######### batch_roi_labels #########")
            # # print(batch_roi_labels.shape)
            # # print("######### batch_roi_ious #########")
            # # print(batch_roi_ious.shape)
            # # print(max_overlaps.shape)
            # print("######### batch_roi_scores #########")
            # print(cur_roi_labels.shape)
            # print(batch_roi_scores.shape)
            # print(cur_roi_scores.shape)
            # print(batch_roi_scores)
            # print(cur_roi_scores)
            
            ### JY: max(sample_inds) > len(cur_roi_scores)
            ## roi 갯수랑 roi score 갯수랑 다름
            ...
            # print(gt_assignment.max())
            # print("################## sampled inds ###############")
            # print(max(sampled_inds))
            # print(min(sampled_inds))
            # print(sampled_inds.shape)
            ...
            # print("################## final batch_gt_of_rois ###############")
            # print(batch_gt_of_rois.shape)
            # print(batch_gt_of_rois)
    ...
    def subsample_rois(self, max_overlaps):
        ...
        if fg_num_rois > 0 and bg_num_rois > 0:
            ...
            bg_inds = torch.tensor([]).todevice(fg_inds.device)

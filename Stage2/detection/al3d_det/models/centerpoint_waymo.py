...
class CenterPointPC(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.pretrain = model_cfg.PRETRAIN
    ...
    def get_training_loss(self, index):
        disp_dict = {}
        tb_dict = {}
        if index == 0:
            if self.pretrain:
        ...
        if self.second_stage:
            if index == 0:
                ...
                loss_rcnn, tb_dict = self.roi_head_ev.get_loss(tb_dict)
                loss = loss_rcnn
    ...
    def build_networks(self):
        ...
        if self.second_stage:
            ...
            roi_head_ev = roi_heads.__all__[self.model_cfg.ROI_HEAD_EV.NAME](
            model_cfg=self.model_cfg.ROI_HEAD_EV,
            input_channels=model_info_dict['num_point_features'],
            num_class=self.num_class if not self.model_cfg.ROI_HEAD_EV.CLASS_AGNOSTIC else 1,
            grid_size=model_info_dict['grid_size'],
            voxel_size=model_info_dict['voxel_size'],
            point_cloud_range=model_info_dict['point_cloud_range']
            )
            self.add_module('roi_head_ev', roi_head_ev)
            module_list.append(roi_head_ev)
            

        return module_list


    def post_processing(self, batch_dict, idx):
            ...
            for index in range(batch_size):
                ...
                if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                    ...
                    if idx == 0:
                        ...
                        # scores = torch.sqrt(batch_dict['roi_scores'][index].reshape(-1))
                        # scores = batch_dict['roi_scores'][index].reshape(-1)    
                    ...
                    if idx == 0:
                        ...
                        selected, selected_scores = nms_utils.class_agnostic_nms(
                            box_scores=score_preds, box_preds=box_preds,
                            nms_config=post_process_cfg.NMS_CONFIG,
                            score_thresh=post_process_cfg.EV_SCORE_THRESH
                        )
    ...
    @staticmethod
    def generate_recall_record(box_preds, recall_dict, batch_index, data_dict=None, thresh_list=None, time_index=None):
        if 'gt_boxes' not in data_dict:
            return recall_dict

        rois = data_dict['rois'][batch_index] if 'rois' in data_dict else None
        
        gt_boxes = data_dict['gt_boxes'][time_index][batch_index]

...
class VoxelAggregationHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict, index):
        ...
        ## hoonhee. image feature를 point에 가져옴.
        ...
        if index == 0:
            ...
            # RoI aware pooling
            # hoonhee, roi_grid_pool = global fusion module (GoF) :)
            pooled_features, global_roi_grid_points, local_roi_grid_points, ball_idxs = self.roi_grid_pool(batch_dict)  # (BxN, 6x6x6, C)
            
            batch_dict['GoF_result_feats'] = pooled_features.clone()
            batch_dict['ball_idxs'] = ball_idxs
            batch_dict['local_roi_grid_points'] = local_roi_grid_points #roi 기준 voxel의 좌표 (PIE  하기 위해 필요한 좌표)
            
            
            batch_size_rcnn = pooled_features.shape[0]
            ### hoonhee -> LoF all module here
            if self.pool_cfg.get('DENSITYQUERY', {}).get('ENABLED'):
                ...
                batch_dict['localgrid_densityfeat'] = localgrid_densityfeat
                ...
                batch_dict['grid_coordid'] = grid_coordid
                batch_dict['grid_coords'] = grid_coords
                ...
                batch_dict['LoF_result_feats'] = localgrid_densityfeat_fuse.clone().permute(0, 2, 1)
                
                if self.pool_cfg.DENSITYQUERY.get('COMBINE'):
                    pooled_features = pooled_features + localgrid_densityfeat_fuse.permute(0, 2, 1)
                    
            ### hoonhee -> self-attention (FDA)
            if self.pool_cfg.get('ATTENTION', {}).get('ENABLED'):
                src_key_padding_mask = None
                
                if 'ball_idxs' in batch_dict.keys():
                    ball_idxs = batch_dict['ball_idxs']
                if 'local_roi_grid_points' in  batch_dict.keys():
                    local_roi_grid_points = batch_dict['local_roi_grid_points']
        ...
        if index != 0:
            ...
            pooled_features = batch_dict['GoF_result_feats']
            batch_size_rcnn = pooled_features.shape[0]
            ...
            # localgrid_densityfeat = self.ffn(localgrid_densityfeat.permute(0, 2, 1))
            # localgrid_densityfeat = localgrid_densityfeat.reshape(-1, self.ffn_hidden_feat)
            localgrid_densityfeat = batch_dict['localgrid_densityfeat']
            ...
            # grid_coordid = torch.cat(grid_coords_idlist, dim=0).to(global_roi_grid_points.device)
            # grid_coords = torch.cat((grid_coordid, global_roi_grid_points.view(-1, 3)), dim=-1)
            grid_coordid = batch_dict['grid_coordid']
            grid_coords = batch_dict['grid_coords']
            
            localgrid_densityfeat_fuse = self.gof_event_crossattention_pointhead(batch_dict, point_features=localgrid_densityfeat, point_coords=grid_coords, layer_name="layer1")
            localgrid_densityfeat_fuse = localgrid_densityfeat_fuse.reshape(pooled_features.shape[0], pooled_features.shape[1], self.ffn_hidden_feat)
            localgrid_densityfeat_fuse = self.up_ffn2(localgrid_densityfeat_fuse.permute(0, 2, 1))
            # pdb.set_trace()
            # pooled_features = self.ev_gof_fusion(localgrid_densityfeat_fuse.permute(0, 2, 1), pooled_features)
            pooled_features = localgrid_densityfeat_fuse.permute(0, 2, 1) + batch_dict['LoF_result_feats']
            # pooled_features = batch_dict['GoF_result_feats'] + batch_dict['LoF_result_feats']

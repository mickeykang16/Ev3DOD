...
from .proposal_target_layer_ev import ProposalTargetLayerEV
import pdb
import copy
...
class RoIHeadTemplate(nn.Module):
    def __init__(self, num_class, model_cfg, **kwargs):
        ...
        self.proposal_target_layer = ProposalTargetLayerEV(roi_sampler_cfg=self.model_cfg.TARGET_CONFIG)
    ...
    def assign_targets(self, batch_dict, index):
        batch_size = batch_dict['batch_size']
        with torch.no_grad():
            targets_dict = self.proposal_target_layer.forward(batch_dict, index)
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if loss_cfgs.REG_LOSS == 'smooth-l1':
            ...
            ) #x,y,z -> 위치에 대해선 loss 0, 크기 dx, dy, dz는 offset이 들어가고. heading에 대해서는 global loss가 들어감
    ...
    def get_box_cls_layer_loss(self, forward_ret_dict):
        ...
        rcnn_cls_labels = forward_ret_dict['rcnn_cls_labels'].view(-1).detach()
        # pdb.set_trace()
        valid_mask = rcnn_cls_labels != float('-inf')
        
        if loss_cfgs.CLS_LOSS == 'BinaryCrossEntropy':
            rcnn_cls_flat = rcnn_cls.view(-1)
            
            batch_loss_cls = F.binary_cross_entropy(torch.sigmoid(rcnn_cls_flat[valid_mask]), rcnn_cls_labels[valid_mask].float(), reduction='none')
            
            #
            # pred_label = torch.sigmoid(rcnn_cls_flat).clip(0, 1)
            # batch_loss_cls = F.binary_cross_entropy(pred_label[valid_mask], rcnn_cls_labels[valid_mask].float(), reduction='none')
            #
            cls_valid_mask = (rcnn_cls_labels[valid_mask] >= 0).float()
...
class VoxelAggregationHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        self.crossattention_head = fusion_modules.__all__[self.pool_cfg.EVFUSION.NAME](
                fuse_mode=self.pool_cfg.EVFUSION.FUSE_MODE, 
                interpolate=False, 
                voxel_size=voxel_size, 
                pc_range=point_cloud_range, 
                image_list=self.pool_cfg.EVFUSION.CAMERAS, 
                image_scale=self.pool_cfg.EVFUSION.IMAGE_SCALE, 
                depth_thres=self.pool_cfg.EVFUSION.DEPTH_THRES, 
                layer_channel=self.pool_cfg.EVFUSION.LAYER_CHANNEL,
                mid_channels=self.pool_cfg.EVFUSION.MID_CHANNELS,
                double_flip=False, 
                dropout_ratio=0,
                activate_out=True,
                fuse_out=True
        ...
        # self.gof_event_crossattention_pointhead = fusion_modules.__all__[self.pool_cfg.GOFEVFUSION.NAME](
        #         fuse_mode=self.pool_cfg.GOFEVFUSION.FUSE_MODE, 
        #         interpolate=False, 
        #         voxel_size=voxel_size, 
        #         pc_range=point_cloud_range, 
        #         image_list=self.pool_cfg.GOFEVFUSION.CAMERAS, 
        #         image_scale=self.pool_cfg.GOFEVFUSION.IMAGE_SCALE, 
        #         depth_thres=self.pool_cfg.GOFEVFUSION.DEPTH_THRES, 
        #         layer_channel=self.pool_cfg.GOFEVFUSION.LAYER_CHANNEL,
        #         mid_channels=self.pool_cfg.GOFEVFUSION.MID_CHANNELS,
        #         double_flip=False, 
        #         dropout_ratio=0,
        #         activate_out=True,
        #         fuse_out=True,
        #         final_fusion_feat=ffn_up_feat
        # )
        ...
        self.GOF_LSTM = img_modules.__all__[self.model_cfg.GOF_LSTM.NAME](
            model_cfg=self.model_cfg.GOF_LSTM
        )

        self.ev_shared_fc_layer = nn.Sequential(*shared_fc_list)

        self.ev_reg_layers = self.make_fc_layers(
            input_channels=pre_channel,
            output_channels=self.box_coder.code_size * self.num_class,
            fc_list=self.model_cfg.REG_FC
        )

        self.ev_cls_layers = self.make_fc_layers(
            input_channels=pre_channel, output_channels=self.num_class, fc_list=self.model_cfg.CLS_FC
        )

        self.gof_ev_fuse_conv = nn.Sequential(
    ...
    def init_weights(self, weight_init='xavier'):
        ...
        nn.init.normal_(self.ev_reg_layers[-1].weight, mean=0, std=0.001)
    ...
    def final_fusion(self, init_fuse, gof_feat):
        fuse_out = torch.cat([init_fuse, gof_feat], dim=-1)
        fuse_out = F.relu(fuse_out)
        fuse_out = self.gof_ev_fuse_conv(fuse_out)
        return fuse_out
    ...
    def forward(self, batch_dict, index):
        ...
        with torch.no_grad():
            if self.training:
            # if True:
                if len(batch_dict['gt_boxes'][0][0]) == 0:
                    batch_dict['rois'] = torch.zeros([batch_dict['batch_size'], 1, 7], device=batch_dict['gt_boxes'][0].device)
                    batch_dict['roi_labels'] = torch.zeros([batch_dict['batch_size'], 1], device=batch_dict['gt_boxes'][0].device).type(torch.int)
                    batch_dict['roi_scores'] = torch.zeros([batch_dict['batch_size'], 1], device=batch_dict['gt_boxes'][0].device)
                    batch_gt_ids = torch.zeros([batch_dict['batch_size'], 1], device=batch_dict['gt_boxes'][0].device)
                else:
                    batch_dict['rois'] = batch_dict['gt_boxes'][0][:, :, :7].clone()
                    batch_dict['roi_scores'] = torch.ones(batch_dict['gt_boxes'][0][:, :, :7].shape[0], batch_dict['gt_boxes'][0][:, :, :7].shape[1], 
                                                        device=batch_dict['gt_boxes'][0].device)
                    batch_dict['roi_labels'] = batch_dict['gt_boxes'][0][:, :, -1].clone().type(torch.int)
                    batch_gt_ids = batch_dict['gt_boxes'][0][:, :, 9].clone()
                    
                
                iou_bg_thresh = self.model_cfg.TARGET_CONFIG.CLS_BG_THRESH
                iou_fg_thresh = self.model_cfg.TARGET_CONFIG.CLS_FG_THRESH
                cls_labels = (torch.ones_like(batch_dict['roi_scores']).long()- iou_bg_thresh) / (iou_fg_thresh - iou_bg_thresh)
                batch_dict['targets_dict'] = {'rois': batch_dict['rois'], 
                                            'gt_iou_of_rois': torch.ones_like(batch_dict['roi_scores']),
                                            'roi_scores': batch_dict['roi_scores'], 
                                            'roi_labels': batch_dict['roi_labels'],
                                            'reg_valid_mask': torch.ones_like(batch_dict['roi_scores']).long(),
                                            'rcnn_cls_labels': cls_labels,
                                            'gt_of_rois': batch_dict['rois'],
                                            'batch_gt_ids': batch_gt_ids
                                            }
            else:
                if batch_dict['rois'].shape[1] == 0:
                    batch_dict['rois'] = torch.zeros([batch_dict['batch_size'], 1, 7], device=batch_dict['rois'][0].device)
                    batch_dict['roi_labels'] = torch.zeros([batch_dict['batch_size'], 1], device=batch_dict['rois'][0].device).type(torch.int)
                    batch_dict['roi_scores'] = torch.zeros([batch_dict['batch_size'], 1], device=batch_dict['rois'][0].device)
                    batch_gt_ids = torch.zeros([batch_dict['batch_size'], 1], device=batch_dict['rois'][0].device)
            
            # batch_dict = self.GOF_LSTM(batch_dict) 
            batch_dict['point_features'], batch_dict['point_coords'] = self.get_point_voxel_features(batch_dict)
            
            if self.training: # TODO
                
                targets_dict = self.assign_targets(batch_dict, index)
                # targets_dict = self.assign_targets(batch_dict, 0)
                # batch_dict['rois'] = targets_dict['rois']
                # batch_dict['roi_labels'] = targets_dict['roi_labels']
                # batch_dict['roi_scores'] = targets_dict['roi_scores'] # hoonhee add
                
            pooled_features, global_roi_grid_points, local_roi_grid_points, ball_idxs = self.roi_grid_pool(batch_dict) 
            # pooled_features = self.final_fusion(pooled_features.reshape(-1, pooled_features.shape[-1])
            #         , batch_dict['GoF_result_feats'].reshape(-1, batch_dict['GoF_result_feats'].shape[-1])) \
            #         .reshape(pooled_features.shape[0], pooled_features.shape[1], pooled_features.shape[2])
            
            batch_size_rcnn = pooled_features.shape[0]
            
            ### hoonhee -> our implicit align module
            src_key_padding_mask = None
            grid_size = self.model_cfg.ROI_GRID_POOL.GRID_SIZE

            shared_features = self.ev_shared_fc_layer(pooled_features.reshape(batch_size_rcnn, -1, 1))
        
        # pdb.set_trace()
        if self.model_cfg.get('TRAIN_EV_CLS_ONLY', False):
            with torch.no_grad():
                rcnn_reg = self.ev_reg_layers(shared_features).transpose(1, 2).contiguous().squeeze(dim=1)  # (B, C)
                rcnn_reg[:, 3:6] = 0.0
        else:
            rcnn_reg = self.ev_reg_layers(shared_features).transpose(1, 2).contiguous().squeeze(dim=1)  # (B, C)
            if not self.model_cfg.LOSS_CONFIG.get('SIZE_EST', False):      
                rcnn_reg[:, 3:6] = 0.0
            # rcnn_reg[:, 3:6] = 0.0
        ...
        rcnn_cls = self.ev_cls_layers(shared_features)
        ...
        # batch_dict['batch_cls_preds'] = batch_dict['init_cls_preds']
        ...
        if self.training:
            ...
            # pdb.set_trace()
            iou_bg_thresh = self.target_config.CLS_BG_THRESH
            iou_fg_thresh = self.target_config.CLS_FG_THRESH
            rcnn_cls_labels = torch.zeros((batch_dict['batch_size'], batch_box_preds.shape[1]), device=rcnn_cls.device)
            for batch in range(batch_dict['batch_size']):
                pred_mask = batch_dict['targets_dict']['batch_gt_ids'][batch] != 0
                gt_mask = batch_dict['gt_boxes'][index][batch][:, 9] != 0
                pred = batch_box_preds[batch][pred_mask]
                gt_box = batch_dict['gt_boxes'][index][batch][:, 0:7][gt_mask].clone()
                iou3d = iou3d_nms_utils.boxes_iou3d_gpu(pred, gt_box)  # (M, N)
                box_pred_id = batch_dict['targets_dict']['batch_gt_ids'][batch][pred_mask]
                gt_id = batch_dict['gt_boxes'][index][batch][:, 9][gt_mask]
                id_index = torch.where(box_pred_id.unsqueeze(1) == gt_id.unsqueeze(0))
                max_overlaps = torch.zeros(batch_box_preds[batch].shape[0], device=batch_box_preds[batch].device)
                max_overlaps[id_index[0]] = iou3d[id_index[0], id_index[1]]
                # if len(pred) > len(gt_box):
                #     pdb.set_trace()
                
                rcnn_cls_labels[batch][id_index[0]] = 1.0
                
                # fg_mask = max_overlaps > iou_fg_thresh
                # bg_mask = max_overlaps < iou_bg_thresh
                # interval_mask = (fg_mask == 0) & (bg_mask == 0)
                # rcnn_cls_labels[batch] = (fg_mask > 0).float()
                # rcnn_cls_labels[batch][interval_mask] = (max_overlaps[interval_mask] - iou_bg_thresh) / (iou_fg_thresh - iou_bg_thresh)
                
                rcnn_cls_labels[batch][~pred_mask] = float('-inf')
                # pdb.set_trace()
            targets_dict['rcnn_cls_labels'] = rcnn_cls_labels.reshape(-1, 1)
            if torch.isnan(targets_dict['rcnn_cls_labels']).any(): 
                pdb.set_trace()
                print(targets_dict['rcnn_cls_labels'])
                print("Pred", batch_box_preds[batch])
                print("GT", batch_dict['gt_boxes'][index][batch])
                print("IOU3D", iou3d_nms_utils.boxes_iou3d_gpu(pred, gt_box))
                print("Seqeunce",  batch_dict['sequence_name'][batch])
                print("frame_id", batch_dict['frame_id'][batch])
...
class LoGoHeadEv(VoxelAggregationHead):
    def __init__(self, input_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):

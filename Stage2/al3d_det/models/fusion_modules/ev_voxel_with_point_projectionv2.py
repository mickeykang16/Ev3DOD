...
class EVVoxelWithPointProjectionV2(nn.Module):
    def __init__(self, 
                fuse_mode, 
                interpolate, 
                voxel_size, 
    ...
    def forward(self, batch_dict, point_features, point_coords, layer_name=None, img_conv_func=None):
        ...
        for cam_key in self.image_list:
            ...
            with CudaTimer(device=final_img_voxels.device, timer_name="point_projector"):
            ...
            for _idx in range(batch_size): #(len(batch_dict['image_shape'][cam_key])):
                with CudaTimer(device=final_img_voxels.device, timer_name="event_voxel_projection"):
                    _idx_key = _idx//tta_num if self.double_flip else _idx
                    image_feat = batch_dict['event_features']['layer1_feat2d'][_idx_key]
                    ...
                    if image_grid[point_mask].shape[0]>1:
                        ...
                        img_voxels[voxel_mask] = torch.nn.functional.grid_sample(image_feat, ref_points, align_corners=False).squeeze().permute(1,0)
                        
                        # N x 64 x 9 -> N x (64 x 9) x 1 x 1 -> 1 x 1 conv or
                        # N x 64 x 9 -> N x (64) x 3 x 3 -> 3 x 3 conv
        ...
        if 'viz_feat' not in batch_dict.keys(): batch_dict['viz_feat']=[]
        viz_feature = {
            'ev_feat': image_feat.cpu().numpy(),
            'ev_voxel_feat': final_img_voxels.cpu().numpy(),
            'point_features': point_features.cpu().numpy(),
            'point_coords': point_coords.cpu().numpy(),
        }
        batch_dict['viz_feat'].append(viz_feature)
                
        return final_img_voxels
                
        # final_voxelimg_feat = self.fusion_withdeform(final_img_voxels, point_features)
        
        # final_voxelimg_feat = final_voxelimg_feat.reshape(batch_dict['GoF_result_feats'].shape[0], batch_dict['GoF_result_feats'].shape[1], -1)
        # final_voxelimg_feat = self.final_fusion(final_voxelimg_feat, batch_dict['GoF_result_feats'].reshape(-1, batch_dict['GoF_result_feats'].shape[-1]).detach())

...
class M3EDInferenceDataset(DatasetTemplate):
    """
    The Dataset class for Inference on M3ED
    """
...
class M3EDTrainingDataset(DatasetTemplate):
    """
    The Dataset class for Training on M3ED (from File System)
    """
    ...
    def __init__(self, dataset_cfg, class_names, root_path, training=True, logger=None) -> None:
        ...
        split_dir = os.path.join(self.root_path, self.split + '.txt')
    ...
    def init_infos(self):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            info_path = os.path.join(self.data_path, 'labels', sequence_name, ('%s.pkl' % 'label')).replace('.pkl', '_interpolate_fov_bbox_lidar_check.pkl')
            ...
            # if self.mode == 'train':
            #     infos = [x for i, x in enumerate(infos) if (i + 1) % 4 != 0]
            # else:
            #     infos = [x for i, x in enumerate(infos) if (i + 1) % 4 == 0]
        ...
        self.logger.info('Total samples for M3ED dataset: %d' % (len(waymo_infos)))
        ...
        if self.dataset_cfg.SAMPLED_INTERVAL[self.mode] > 1:
            ...
            self.logger.info('Total sampled samples for M3ED dataset: %d' % len(self.infos))
    ...
    def get_images_and_params(self, current_idx, idx_list):
        ...
        for i in idx_list:
            ...
            for key in img_infos.keys():
                if 'path' not in key or 'image_0' not in key: continue
                img_path = img_infos[key]
                for j in range(1):
                    # pdb.set_trace()

...
class CenterPointMM_TIME(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        super().__init__()
    ...
    def forward(self, batch_dict):
        ...
        ret_lidar = None
        with CudaTimer(device=device_, timer_name="vfe"):
            batch_dict = self.lidar(batch_dict, self.lidar.module_list[0])
        with CudaTimer(device=device_, timer_name="backbone3d"):
            batch_dict = self.lidar(batch_dict, self.lidar.module_list[1])
        with CudaTimer(device=device_, timer_name="map_to_bev"):
            batch_dict = self.lidar(batch_dict, self.lidar.module_list[2])
        with CudaTimer(device=device_, timer_name="backbone2d"):
        ...
        #     'aug_mat': aug_mat
        # })
        with CudaTimer(device=device_, timer_name="dense_head"):
        ...
        for index in range(10):
            ...
            if index == 0:
                if self.second_stage:
                    with CudaTimer(device=device_, timer_name="roi_head"):
                ...
                # if self.second_stage:
                with CudaTimer(device=device_, timer_name="roi_head_ev"):

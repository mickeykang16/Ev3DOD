...
class VoxelAggregationHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict, index):
        ...
        # batch_dict['GoF_result_feats'] = pooled_features.requires_grad_(True)
        batch_dict['GoF_result_feats'] = pooled_features
        batch_dict['ball_idxs'] = ball_idxs
        # batch_dict['local_roi_grid_points'] = local_roi_grid_points.requires_grad_(True) #roi 기준 voxel의 좌표 (PIE  하기 위해 필요한 좌표)
        batch_dict['local_roi_grid_points'] = local_roi_grid_points
        ...
        if self.pool_cfg.get('DENSITYQUERY', {}).get('ENABLED'):
            ...
            batch_dict['localgrid_densityfeat'] = localgrid_densityfeat.requires_grad_(True)

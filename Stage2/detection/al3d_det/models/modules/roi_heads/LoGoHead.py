...
class VoxelAggregationHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict, index):
        ...
        batch_dict['targets_dict'] = targets_dict
        ...
        batch_dict['GoF_result_feats'] = pooled_features.detach()
        batch_dict['ball_idxs'] = ball_idxs.detach()
        batch_dict['local_roi_grid_points'] = local_roi_grid_points.detach()
        ...
        if self.pool_cfg.get('DENSITYQUERY', {}).get('ENABLED'):
            ...
            batch_dict['localgrid_densityfeat'] = localgrid_densityfeat.detach()
            ...
            # batch_dict['LoF_result_feats'] = localgrid_densityfeat_fuse.clone().permute(0, 2, 1).requires_grad_(True)
        ...
        batch_dict['rcb_shared_features'] = shared_features.detach()
        ...
        # batch_dict['rois'] = rcnn_reg
        # if self.training:
        #     rcnn_roi2 = targets_dict['gt_of_rois_src'].clone().detach()
        # else:
        #     roi_boxes3d = targets_dict['rois'].clone().detach()
        #     roi_ry = roi_boxes3d[:, :, 6].view(-1)
        #     roi_xyz = roi_boxes3d[:, :, 0:3].view(-1, 3)
            
        #     rois_anchor = roi_boxes3d.clone()
        #     rois_anchor[:,:, 0:3] = 0
        #     rcnn_roi = self.box_coder.decode_torch(
        #         rcnn_reg.reshape(rois_anchor.shape[0], rois_anchor.shape[1], -1), rois_anchor
        #     ).view(-1, rcnn_reg.shape[-1])
            
        #     rcnn_roi2 = common_utils.rotate_points_along_z(
        #                 rcnn_roi.unsqueeze(dim=1), roi_ry
        #             ).squeeze(dim=1)
        #     rcnn_roi2[:, 0:3] += roi_xyz
        #     rcnn_roi2 = rcnn_roi2.reshape(roi_boxes3d.shape)
        ...
        if not self.training:
            ...
            batch_dict['init_cls_preds'] = batch_cls_preds
            batch_dict['batch_box_preds'] = batch_box_preds
            batch_dict['cls_preds_normalized'] = False
            
            # batch_dict['rois'] = rcnn_roi2
